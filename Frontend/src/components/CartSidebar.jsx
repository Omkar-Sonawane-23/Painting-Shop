...
const CartSidebar = ({ isOpen, onClose, cartItems, onRemove, onCheckout }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[60] ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      ...
      {/* Sidebar */}
      <div className={`absolute top-0 right-0 w-full max-w-md h-full bg-zinc-950 border-l border-zinc-800 shadow-2xl transform transition-transform duration-300 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item, idx) => (
              <div key={`${item.id}-${idx}`} className="flex gap-4 bg-zinc-900/50 p-3 rounded-xl border border-zinc-800/50 group">
                <div className="h-16 w-16 rounded-lg bg-zinc-800 shadow-sm flex-shrink-0" style={{ background: item.imageColor }}></div>
                <div className="flex-1 flex flex-col justify-between">
                  ...
                  <div className="flex justify-between items-end">
                    ...
                    <button onClick={() => onRemove(idx)} className="text-zinc-600 hover:text-red-500 transition-colors">
                        <Trash2 size={14} />
                    </button>
                  </div>
                </div>
