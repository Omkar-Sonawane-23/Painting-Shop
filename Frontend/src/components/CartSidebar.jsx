    ...
    <div className={`fixed inset-0 z-[100] ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      ...
      <div className={`absolute top-0 right-0 w-full max-w-md h-full bg-[#FAF9F6] border-l-2 border-zinc-800 shadow-2xl transform transition-transform duration-300 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="p-6 border-b-2 border-zinc-800 flex justify-between items-center bg-[#FAF9F6]">
          <h2 className="text-xl font-black text-zinc-800 uppercase tracking-wide flex items-center gap-2 font-sans">
            Your Cart <span className="text-sm text-zinc-500 font-sans normal-case">({cartItems.length} Items)</span>
          </h2>
          <button onClick={onClose} className="text-zinc-400 hover:text-red-500 transition-colors p-2 hover:bg-zinc-200 rounded-full">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item, idx) => (
              <div key={`${item.id}-${idx}`} className="flex gap-4 bg-white p-4 rounded-xl border-2 border-zinc-200 shadow-sm relative group">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <h4 className="text-zinc-900 font-bold text-sm line-clamp-1 font-sans text-lg">{item.name}</h4>
                  ...
                  <div className="flex justify-between items-end mt-2">
                    ...
                    <div className="flex items-center bg-white border-2 border-zinc-200 rounded-lg h-8">
                       <button 
                         ...
                         className="px-2 h-full hover:bg-zinc-100 text-zinc-500 disabled:opacity-50 flex items-center"
                       ...
                       <button 
                         onClick={() => onUpdateQuantity(idx, 1)}
                         className="px-2 h-full hover:bg-zinc-100 text-zinc-500 flex items-center"
                       >
                         <Plus size={12} />
                       </button>
                    </div>
                ...
                <button 
                   ...
                   className="absolute -top-2 -right-2 bg-white text-zinc-400 hover:text-red-500 border-2 border-zinc-200 p-1.5 rounded-full shadow-sm transition-colors z-10"
                >
                    <Trash2 size={14} />
                </button>
              </div>
